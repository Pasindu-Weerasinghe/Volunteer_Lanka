...
class ComplaintsModel extends Model
{
    ...
    function getComplaintDetails(){
        $query = "SELECT complaints.C_ID, complaints.About, complaints.U_ID, complaints.Complain, complaints.Response, user.Role FROM complaints INNER JOIN user on complaints.U_ID = user.U_ID;";
        $statement = $this->db->prepare($query);
        $statement->execute();
        return $statement->fetchAll(PDO::FETCH_ASSOC);
    }
